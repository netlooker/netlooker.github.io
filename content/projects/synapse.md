...
Synapse is live and usable. It indexes generic markdown folders, stores note and chunk embeddings, runs semantic search in `note`, `chunk`, and `hybrid` modes, and exposes aligned MCP and HTTP interfaces. The current lane focuses on tuning discovery quality on larger corpora, expanding vector-index audit inside `Cipher`, and preparing the seam for a second backend beyond `sqlite-vec`.
...
Jack in, index the vault, and let the grid start whispering back.
